...
export const MagneticCursor: FC<MagneticCursorProps> = ({
  ...
  shape = "circle",
  disableOnTouch = true,
  // More fluid defaults: faster stretch response + bigger scale range so
  // the cursor reads as a viscous droplet rather than a hard disc.
  ...
  useEffect(() => {
    ...
    magneticElements.forEach((el) => {
      ...
      const handlePointerEnter = () => {
        ...
        // Liquid splash-into-shape: slightly overshoot on entry, then settle.
        gsap.to(cursorEl, {
          x: centerX,
          y: centerY,
  ...
  const styles: React.CSSProperties = {
    ...
    WebkitBackdropFilter:
      contrastBoost !== 1 ? `contrast(${contrastBoost})` : "none",
    // Soft edge → reads as a viscous droplet rather than a hard pixel disc.
    filter: "blur(0.6px)",
  };
